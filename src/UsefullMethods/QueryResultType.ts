import { IBaseModule, IChildLoader, IId, IQueryResultItem, IDatabase } from "../sql.wrapper.types";
...
const createQueryResultType = async function <T extends IId<D>, D extends string>(item: any, database: IDatabase<D>, children?: IChildLoader<D>[]): Promise<IQueryResultItem<T, D>> {
    ...
    result.update = async (...keys: any[]) => {
        if (!keys || keys.length <= 0)
            return;
        const kItem: any = { tableName: (result as any).tableName, id: (result as any).id } as IBaseModule<any>;
        keys.forEach(k => {
            kItem[k] = result[k];
        });
